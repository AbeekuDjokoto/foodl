  ...
  <div class="container">
    ...
    <div class="cardFlex">
      <div v-for="category in categories" :key="category.idMeal" class="card">
        <figure>
          <router-link :to="'/about/' + category.idMeal"
            ><img :src="category.strMealThumb" alt="" class="foodImg"
          /></router-link>
...
export default {
  ...
  methods: {
    async searchResult() {
      await axios
        .get(
          `https://www.themealdb.com/api/json/v1/1/filter.php?i=${this.result}`
        )
        .then((response) => {
